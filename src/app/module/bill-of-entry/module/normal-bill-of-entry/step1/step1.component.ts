...
export class Step1Component implements OnInit, ControlValueAccessor, Validator {
  ...
  writeValue(val: any): void {
    ...
    val && this.homeConsumptionFormStep1.patchValue(val, { emitEvent: true });
  }
  registerOnChange(fn: any): void {
    //console.log("on change");
    this.homeConsumptionFormStep1.valueChanges.subscribe(fn);
  }
  ...
  setDisabledState?(isDisabled: boolean): void {
    isDisabled ? this.homeConsumptionFormStep1.disable() : this.homeConsumptionFormStep1.enable();
  }
  validate(c: AbstractControl): ValidationErrors | null {
    //console.log("Consignment Info validation", c);
    return this.homeConsumptionFormStep1.valid ? null : { invalidForm: { valid: false, message: "Step1 fields are invalid" } };
  }
  ...
  isFieldValid(field: string) {
    return (
      (!this.homeConsumptionFormStep1.get(field).valid && this.homeConsumptionFormStep1.get(field).touched) ||
      (this.homeConsumptionFormStep1.get(field).untouched && this.formSumitAttempt)
    );
  }
